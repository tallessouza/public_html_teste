...
class AIController extends Controller
{
    ...
    public function stream(Request $request)
    {
        ...
        $data = match ($default_ai_engine) {
            'openai' => $this->chatGptStreamData($request),
            'anthropic' => $this->anthropicStreamData($request),
            default => $this->chatGptStreamData($request),
        };
    ...
    public function anthropicStreamData(Request $request)
    {
        $messages = [[
            'role' => 'user',
        ...
        foreach (explode("\n", $data) as $chunk) {
            ...
            if (isset(json_decode($chunk)->delta->text)) {
                ...
                $text = json_decode($chunk)->delta->text;

                $messages[] = json_encode(['message' => $text]);
            }
        }
    ...
    public function chatGptStreamData(Request $request)
    {
        $client = OpenAI::factory()
            ->withApiKey(Helper::setOpenAiKey())
            ->make();
        ...
        foreach ($result as $response) {
            $data[] =json_encode(['message' => $response->choices[0]->delta->content]);
        }
