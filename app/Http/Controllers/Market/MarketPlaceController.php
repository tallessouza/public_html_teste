...
class MarketPlaceController extends Controller
{
    ...
    public function setLicenses(): void
    {
        try {
            $data = Http::withHeaders([
                'x-domain-key' => Helper::settingTwo('liquid_license_domain_key'),
                'x-domain' => request()->getHost(),
            ])->post('http://magicmarket.liquid-themes.com/api/request', [
                'domain' => request()->getHost(),
                'domain_key' => Helper::settingTwo('liquid_license_domain_key'),
                'extension' => Extension::query()->get()->toArray()
            ]);
